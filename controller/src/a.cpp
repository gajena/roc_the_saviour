...
float k_p = 0.06, k_i = 0.000075, k_d = 0.01;
float sp_thresh = 0.15, err_sum_x = 0.0, err_sum_y = 0.0;
float yaw = 5.7;
    ...
    while ( ros::ok() )
    {
		...
		if( aruco_detected_flag == 1)        
		{
	        ...
	        if (i==0)
	         ...
	         { 
		     	mocap.pose.position.y = (x-x_des)*k_p + (err_sum_x)*k_i + (x-x_prev)*k_d;//roll
	            mocap.pose.position.x = (y-y_des)*k_p + (err_sum_y)*k_i + (y-y_prev)*k_d;//pitch
	         } 
	        
	        setpoint.pose.position.z = 1.3f;
	         //mocap.pose.position.x =  0.0f;
	      //  mocap.pose.position.y = 0.025f ;
			mocap.pose.position.z = z;
			dist.pose.position.z = z;
		...
		else
		{
			...
			mocap.pose.position.y = 0; //roll
			mocap.pose.position.x = 0; //pitch


			setpoint.pose.position.z = 1.3f;
			//mocap.pose.position.x =  0.0f;
			//  mocap.pose.position.y = 0.025f ;
			mocap.pose.position.z = 1.3f;
			dist.pose.position.z = 1.3f;
		}
